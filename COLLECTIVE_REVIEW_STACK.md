...
Layers 2 through 5 introduce the generic service transport in four reviewable steps. Its ordinary MTU
...
| `coll-work-2` | `coll-work-1b` | Request and router-event ownership, transport metadata and serialization fixes |
| `coll-work-3` | `coll-work-2` | Network-service negotiation on REPORT_ID, capability query, tagged carriage, pass processor |
| `coll-work-4` | `coll-work-3` | Synthetic egress: bounded requester, service-aware LRU and RR arbitration, output capacity checks |
| `coll-work-5` | `coll-work-4` | Processor-owned ingress: owned VNs, staged transfers, inspect and consume |
| `coll-work-6` | `coll-work-5` | Scalar processor, ordered reduction timing, lazy fanout |
| `coll-work-7` | `coll-work-6` | Shared fat-tree connectivity, sparse plans, validation and idle checkpoint tests |
| `coll-work-8` | `coll-work-7` | Mercury named VNs and ordinary transport tests |
| `coll-work-9` | `coll-work-8` | Mercury/Mask-MPI offload, native memory timing and job binding |
| `coll-work-10` | `coll-work-9` | Firefly/Ember offload, native DMA timing and sparse job tests |
| `coll-work-11` | `coll-work-10` | Existing ARM64 context correction, kept separate from AI demos |
| `coll-work-12` | `coll-work-11` | LRU mixed-traffic experiment fixture, runner, metric tests and this guide |

The initial review ends at `coll-work-12`. AI demos are preserved as the
separate follow-up `coll-work-13`, based on `coll-work-12`; they are not
part of the initial implementation diff.

`coll-work-10` aliases `coll-work-10`: the shared endpoint implementation is now
introduced with its contract in 1b and tested with its consumers. The former
`coll-work-13` aliases `coll-work-13`: expanded regressions now live
...
default and updating this guide, the former layer 2 and later source trees are unchanged.
...
The former layer 2 is now four layers: request and router-event plumbing fixes
(`coll-work-2`), service negotiation and tagged carriage (`coll-work-3`),
synthetic egress with service-aware arbitration (`coll-work-4`), and
processor-owned ingress (`coll-work-5`). Every later layer is renumbered
sequentially; the `coll-work-7` and `coll-work-8-expanded-tests` aliases are
retired and `coll-work-fixes` still names the contention layer. The probe
processor and endpoint moved to `coll-work-8` with the Mercury test that uses
them, and the generic-source boundary test moved to `coll-work-6` with the
first collective code. Validation is recorded in the workspace sibling
`split-20260922`; earlier tips are retained under `backup/split-20260922/`.
