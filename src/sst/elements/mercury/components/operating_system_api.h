...
class OperatingSystemAPI : public SST::Hg::SubComponent {

public:
  SST_ELI_REGISTER_SUBCOMPONENT_API(SST::Hg::OperatingSystemAPI)

  OperatingSystemAPI(ComponentId_t id, SST::Params &params);

  virtual ~OperatingSystemAPI() {}

  virtual void requireLibraryForward(std::string library) = 0;

  virtual Link *selfEventLink() = 0;
  ...
  const SST::Collective::AcceptedParticipantHandle* collectiveParticipant(
   ...
   virtual void setNumRanks(unsigned int ranks) = 0;

   virtual unsigned int numRanks() = 0;

   virtual void setRanksPerNode(unsigned int npernode) = 0;

   virtual unsigned int ranksPerNode() = 0;
  ...
  virtual void startThread(Thread *t) = 0;
    ...
    * @param req [in-out] A key that will store blocking data that will be
    * needed for later unblocking
    * @return
    */
    ...
    * @param id
    * @return The mutex object corresponding to the ID. Return NULL if no mutex
    * is found.
    */
