    ...
    Component(id), rng(id, 13)
{
    ...
    clock_ticks = 0;
    currentFaultRate = 0.1;  // Initialize fault rate
...
void FaultInjCPU::handleCpuEvent(SST::Event *ev)
{
...
void FaultInjCPU::init(unsigned int phase)
{
    memory->init(phase);
}

void FaultInjCPU::setup() {
    memory->setup();
    lineSize = memory->getLineSize();
}

void FaultInjCPU::finish() { }

// incoming events are scanned and deleted
void FaultInjCPU::handleEvent(StandardMem::Request *req)
{
    ...
    if ( requests.end() == i ) {
        ...
        out.output("Received Response (%s), Took: lol, %6zu pending requests.\n",
...
bool FaultInjCPU::clockTic( Cycle_t )
{
    // Send fault injection commands every 100 clock cycles
    // Demonstrates different PM command types with varying parameters
    if (clock_ticks % 100 == 0 && clock_ticks > 0) {
        ...
        switch (commandType) {
            case 0:
                // Single parameter: injection_rate <rate>
                ...
                break;
            case 1:
                // Two parameters: set_range <min> <max> (encoded as "set_range <min>" with rate as max)
                ...
                break;
            case 2:
                // Three parameters: config <addr> <size> (encoded in fname)
        ...
        // Update rate for next injection_rate command
        currentFaultRate += 0.05;
        if (currentFaultRate >= 1.0) {
    ...
    if ( 0 == ops && requests.empty() ) {
        out.verbose(CALL_INFO, 1, 0, "FaultInjCPU: Test Completed Successfuly\n");
...
StandardMem::Request* FaultInjCPU::createWrite(Addr addr) {
...
StandardMem::Request* FaultInjCPU::createRead(Addr addr) {
    addr = ((addr % maxAddr)>>2) << 2;
...
StandardMem::Request* FaultInjCPU::createFlush(Addr addr) {
...
StandardMem::Request* FaultInjCPU::createFlushInv(Addr addr) {
    addr = ((addr % (maxAddr - noncacheableSize)>>2) << 2);
    if (addr >= noncacheableRangeStart && addr < noncacheableRangeEnd)
        addr += noncacheableRangeEnd;
    addr = addr - (addr % lineSize);
...
StandardMem::Request* FaultInjCPU::createFlushCache() {
    StandardMem::Request* req = new Interfaces::StandardMem::FlushCache();
    return req;
}
StandardMem::Request* FaultInjCPU::createLL(Addr addr) {
...
StandardMem::Request* FaultInjCPU::createSC() {
    std::vector<uint8_t> data;
    data.resize(4);
...
StandardMem::Request* FaultInjCPU::createMMIOWrite() {
...
StandardMem::Request* FaultInjCPU::createMMIORead() {
