...
void FourStateAgent::publishState(int kernel)
{
    ...
    if (verbose_) {
        out_->output("FourStateAgent[%s]: publish currentKernel=%d pipelineCycle=%d\n",
                     stateKey_.c_str(), s->currentKernel, s->pipelineCycle);
    }
}
...
{
    // Publish BEFORE actually sending the response so that any PortModule
    // observing the lowlink already sees currentKernel=value on the very
    // first load/store the CPU issues after it unblocks. If we published
    // after the send, there is a race between the response's arrival at
    // the CPU and the first handler memory op.
    //
    // value < 0 (exit sentinel) means "no kernel running", publish IDLE.
