...
/* Constructor */
CarcosaCPU::CarcosaCPU(ComponentId_t id, Params& params) :
...
void CarcosaCPU::setup() {
    memory->setup();
    lineSize = memory->getLineSize();
}

void CarcosaCPU::finish() { }

// incoming events are scanned and deleted
void CarcosaCPU::handleEvent(StandardMem::Request *req)
{
...
bool CarcosaCPU::clockTic( Cycle_t )
{
    ...
    if ( 0 == ops && requests.empty() ) {
        out.verbose(CALL_INFO, 1, 0, "CarcosaCPU: Test Completed Successfuly\n");
...
StandardMem::Request* CarcosaCPU::createWrite(Addr addr) {
...
StandardMem::Request* CarcosaCPU::createRead(Addr addr) {
    addr = ((addr % maxAddr)>>2) << 2;
...
StandardMem::Request* CarcosaCPU::createFlush(Addr addr) {
...
StandardMem::Request* CarcosaCPU::createFlushInv(Addr addr) {
    addr = ((addr % (maxAddr - noncacheableSize)>>2) << 2);
    if (addr >= noncacheableRangeStart && addr < noncacheableRangeEnd)
        addr += noncacheableRangeEnd;
    addr = addr - (addr % lineSize);
...
StandardMem::Request* CarcosaCPU::createFlushCache() {
    StandardMem::Request* req = new Interfaces::StandardMem::FlushCache();
    return req;
}
StandardMem::Request* CarcosaCPU::createLL(Addr addr) {
...
StandardMem::Request* CarcosaCPU::createSC() {
    std::vector<uint8_t> data;
    data.resize(4);
...
StandardMem::Request* CarcosaCPU::createMMIOWrite() {
...
StandardMem::Request* CarcosaCPU::createMMIORead() {
