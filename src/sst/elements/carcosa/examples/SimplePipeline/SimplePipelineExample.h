...
/**
 * Clocked component that advances a 4-stage FSM and, on every transition,
 * writes its stage id + pipeline-cycle count into its PipelineStateBase
 * snapshot published under `state_key`.
 *
 * On each tick it also emits one SimpleStageEvent on the `out` port so there
 * is an event stream for downstream PortModules (such as SimpleStageGate) to
 * intercept.
 */
...
/**
 * Port-side "stage gate": a PortModule that drops events whenever the
 * upstream producer's published PipelineStateBase::currentKernel matches one
 * of the configured `drop_stages` ids.
 *
 * The snapshot pointer is cached lazily because the producer's registry entry
 * is created in its constructor and the gate's constructor may run first.
 */
