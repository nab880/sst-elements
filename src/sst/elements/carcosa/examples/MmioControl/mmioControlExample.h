...
/**
 * Minimal InterceptionAgentAPI demonstrating the transport-neutral control
 * hook. Protocol on the control region:
 *   - read  @ kValueOffset: returns the most-recently-armed value, but BLOCKS
 *                           (Deferred) until a value has been armed;
...
/**
 * Scripted StandardMem requestor that drives an MMIO control peripheral
 * (carcosa.Hali in MMIO mode hosting ExampleControlAgent) and checks the
 * Deferred round-trip:
 *   tick 0: Read value@kValueOffset  -> peripheral parks it (no response yet)
 *   tick 1: Write kArmOffset = arm_value -> arms it, completes the parked read
 *   tick 2+: once the read response arrives, verify it carried arm_value and
 *            end the simulation (with a watchdog so a stuck read still exits).
 */
