...
// FNV-1a 64-bit fold (same construction as VlaGpuWeightAgent) so the bridge's
// action checksum is comparable across the mini-GPU and balar substrates: each
// D2H response is hashed and summed (not XORed) into the running checksum, making
// the fold order-independent and non-cancelling.
static constexpr uint64_t kFnvOffset = 1469598103934665603ULL;
static constexpr uint64_t kFnvPrime  = 1099511628211ULL;
static inline uint64_t fnv1a(uint64_t seed, const uint8_t* p, size_t n) {
    uint64_t h = seed;
...
// ---------------------------------------------------------------------------
// CUDA-API trace parser (adapted from balar's forked test CPU (balar/testcpu/)).
//
// One behavioral change from balar's forked test CPU: for an H2D memcpy the weight payload
// is handed to the bridge (pending_weight_payload_) to stage into the DEDICATED
// weightStageAddr_ region, and cuda_memcpy.src points there -- instead of being
// appended right after the command packet in scratch. That separation is what lets
// EccGuard confine injection to weights only.
// ---------------------------------------------------------------------------
...
void BalarRingBridge::onCacheReadResp(StandardMem::ReadResp* resp)
{
    ...
    if (it->second.first == "Read_CUDA_ret_packet") {
        ...
        requests_.erase(it);
        delete resp;
        issueNextPacket();       // advance the trace (or finish the replay)
        return;
    }
...
void BalarRingBridge::completeCudaCall(const BalarCudaCallReturnPacket_t* ret_pack)
{
    if (ret_pack->cuda_call_id == CUDA_REG_FAT_BINARY && trace_parser_) {
        ...
        // Fold the GPU D2H result into the action checksum. This is the consumed
        // output that diverges when a corrupted weight escaped SECDED (baseline
        // none reproduces the golden checksum).
        size_t n = ret_pack->cudamemcpy.size;
        volatile uint8_t* sim_ptr = ret_pack->cudamemcpy.sim_data;
        if (sim_ptr && n) {
            uint64_t h = fnv1a(kFnvOffset, (const uint8_t*)sim_ptr, n);
            checksum_ += h;
...
void BalarRingBridge::finishReplay()
{
    ...
    // Drain any Cmds that arrived mid-replay.
    if (cmd_pending_ > 0) {
        --cmd_pending_;
        if (replayEachCmd_) beginTrace();
        else if (ringLink_) ringLink_->send(new HaliEvent(RingTag::Done, 0u));
    }
}
