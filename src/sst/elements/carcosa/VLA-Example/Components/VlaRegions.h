...
// Shared helpers for VLA agents to publish workload-labeled memory regions
// (weights, kv_cache, activations, action_queue, ...) into the
// PipelineStateRegistry so EccGuard can drive a region-aware ECC policy.
//
// Region 0 is reserved for the MMIO control window each agent already
// publishes via publishMmioRegion(); user-supplied regions populate slots 1..N
// in the order they appear in the CSV.
//
// CSV format: 'NAME:BASE:SIZE,NAME:BASE:SIZE,...'
//   BASE / SIZE accept hex (0x...) or decimal.
//   Names are lower-case-by-convention but stored verbatim and matched
//   exactly against EccGuard's kernel_policy '*@NAME' / 'KERNEL@NAME' tags.
...
/**
 * Publishes user-supplied (name, base, size) regions into slots 1..N of the
 * snapshot keyed by `stateKey`. Returns the number of regions published.
 *
 * No-op if `stateKey` is empty or the CSV is empty. If a snapshot does not
 * yet exist for `stateKey` it is created.
 */
...
/**
 * Pops PipelineStateBase::frameAbortRequested. Returns true exactly once per
 * abort assertion. Designed for VLA agents to drain on every status-write so
 * one DUE => one frame drop, not a cascade.
 */
