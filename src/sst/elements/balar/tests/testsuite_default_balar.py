...
## Balar integration tests (balar-integration branch)
## Requires: CUDA_INSTALL_PATH, GPGPUSIM_ROOT, NVCC (see quetz-docker/README-balar.md)
...
class testcase_balar_smoke(SSTTestCase):
    ...
    def test_balar_quetz_testcpu_registered(self):
        """sst-info balar lists QuetzTestCPU when the element is built."""
        text, rc = self._sst_info_text()
        if "UNABLE TO PROCESS LIBRARY" in text.upper() or rc != 0:
            self.skipTest("balar element not built (configure without --with-gpgpusim)")
        self.assertIn("QuetzTestCPU", text, "sst-info balar should list balar.QuetzTestCPU")
...
class testcase_balar_simple(BalarTestCase):
    ...
    @BalarTestCase.balar_basic_unittest
    def test_quetz_balar_doorbell(self):
        self.quetz_contract_testcpu_template(
            "quetz_doorbell", "testQuetz-balar-doorbell.py", None, testtimeout=60 * 10)

    @BalarTestCase.balar_basic_unittest
    def test_quetz_balar_malloc_free(self):
        self.quetz_contract_testcpu_template(
            "quetz_malloc_free", "testQuetz-balar-malloc-free.py", "malloc_free.trace", testtimeout=60 * 15)

    @BalarTestCase.balar_basic_unittest
    def test_quetz_balar_wide_packet(self):
        self.quetz_contract_testcpu_template(
            "quetz_wide_packet", "testQuetz-balar-wide-packet.py", "wide_memcpy_d2h.trace",
