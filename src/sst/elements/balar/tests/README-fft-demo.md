...
1. **Trace replay** (`DoorbellTestCPU` / `BalarTestCPU`) — fast, deterministic, CI.
...
| `testBalar-fft.py`, `testDoorbellCPU-fft.py` | trace-replay SDLs |
...
sst testDoorbellCPU-fft.py --model-options=\
...
independent DFT:
```sh
# DoorbellTestCPU/BalarTestCPU dump the GPU D2H result when FFT_DUMP=1
FFT_DUMP=1 sst testDoorbellCPU-fft.py --model-options=\
...
sst testDoorbellCPU-fft.py --model-options="... -x ./balar_trace/fft -t traces/fft_impulse_256.trace" -s staged.stats   # staged main()
