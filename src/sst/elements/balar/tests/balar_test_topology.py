"""
Shared SST topology builders for Balar and Quetz-pattern contract tests.
"""
...
def build_quetz_topology(balar_builder, cfg_file, mode="trace", balar_verbosity=0, dma_verbosity=0):
    ...
    """
    # TODO: hard-coded high MMIO aperture so wide scratch packets cannot overflow
    # into the MMIO region. Should derive from max packet size + alignment instead.
    ...
    cpu = sst.Component("cpu", "balar.QuetzTestCPU")
    cpu.addParams({
        "clock": CLOCK,
        "verbose": balar_verbosity,
        "scratch_mem_addr": 0,
    ...
    chiprtr = sst.Component("quetz_chiprtr", "merlin.hr_router")
    chiprtr.addParams({
        "xbar_bw": "1GB/s",
        "id": "0",
        "input_buf_size": "1KB",
