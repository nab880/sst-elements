...
####
    ...
    def quetz_contract_testcpu_template(
        ...
        if min_cuda_calls_completed > 0:
            ...
            self.assertGreaterEqual(
                ...
                "quetz contract {0}: cuda_calls_completed {1} < {2}".format(
        ...
        if min_d2h_bytes > 0:
            ...
            self.assertGreaterEqual(
                ...
                "quetz contract {0}: total_memD2H_bytes {1} < {2}".format(
                    testcase, d2h_bytes, min_d2h_bytes))
        if min_d2h_ratio > 0.0:
            self._assert_min_d2h_ratio(statsfile, testcase, "quetz contract", min_d2h_ratio)
