...
#include "testcpu/quetzTestCPU.h"
#include "util.h"
...
class QuetzTestCPU::CacheHandlers : public StandardMem::RequestHandler {
public:
    friend class QuetzTestCPU;
    CacheHandlers(QuetzTestCPU* cpu, SST::Output* out) : StandardMem::RequestHandler(out), cpu_(cpu) {}
...
class QuetzTestCPU::MmioHandlers : public StandardMem::RequestHandler {
public:
    friend class QuetzTestCPU;
    MmioHandlers(QuetzTestCPU* cpu, SST::Output* out) : StandardMem::RequestHandler(out), cpu_(cpu) {}
...
class QuetzTestCPU::CudaAPITraceParser {
public:
    CudaAPITraceParser(QuetzTestCPU* cpu, SST::Output* out, const std::string& trace_file, const std::string& cuda_executable)
...
QuetzTestCPU::QuetzTestCPU(ComponentId_t id, Params& params) : Component(id)
{
    out.init("QuetzTestCPU[@f:@l:@p] ", params.find<unsigned int>("verbose", 1), 0, Output::STDOUT);
    ...
    clock_handler_ = new Clock::Handler<QuetzTestCPU, &QuetzTestCPU::clockTic>(this);
    ...
    cache_link_ = loadUserSubComponent<StandardMem>(
        ...
        new StandardMem::Handler<QuetzTestCPU, &QuetzTestCPU::handleCacheEvent>(this));
    mmio_link_ = loadUserSubComponent<StandardMem>(
        "mmio_link", ComponentInfo::SHARE_NONE, clockTC_,
        new StandardMem::Handler<QuetzTestCPU, &QuetzTestCPU::handleMmioEvent>(this));

    if (!cache_link_ || !mmio_link_) {
        out.fatal(CALL_INFO, -1, "QuetzTestCPU requires cache_link and mmio_link subcomponents\n");
    }
...
void QuetzTestCPU::init(unsigned int phase)
{
    cache_link_->init(phase);
    mmio_link_->init(phase);
}

void QuetzTestCPU::setup()
{
    cache_link_->setup();
    mmio_link_->setup();
}

void QuetzTestCPU::finish() {}

void QuetzTestCPU::emergencyShutdown()
{
    if (out.getVerboseLevel() > 1) {
        out.output("QuetzTestCPU %s outstanding=%zu\n", getName().c_str(), requests_.size());
    }
}

void QuetzTestCPU::handleCacheEvent(StandardMem::Request* req) { req->handle(cache_handlers_); }
void QuetzTestCPU::handleMmioEvent(StandardMem::Request* req) { req->handle(mmio_handlers_); }

void QuetzTestCPU::uint64ToData(uint64_t num, std::vector<uint8_t>* data)
{
    data->clear();
...
uint64_t QuetzTestCPU::dataToUInt64(std::vector<uint8_t>* data)
{
    uint64_t retval = 0;
...
void QuetzTestCPU::beginPacketIssue(const BalarCudaCallPacket_t& pack)
{
...
void QuetzTestCPU::sendNextScratchChunk()
{
...
void QuetzTestCPU::sendNextFlush()
{
...
void QuetzTestCPU::sendDoorbell()
{
...
void QuetzTestCPU::sendStartCudaRetRead()
{
...
void QuetzTestCPU::sendReadRetPacket(uint64_t ret_addr)
{
...
void QuetzTestCPU::onCacheWriteReq(StandardMem::Write* write)
{
    if (!write->posted) {
...
void QuetzTestCPU::onCacheWriteResp(StandardMem::WriteResp* resp)
{
...
void QuetzTestCPU::onCacheFlushResp(StandardMem::FlushResp* resp)
{
    auto it = requests_.find(resp->getID());
    if (it == requests_.end()) {
...
void QuetzTestCPU::onCacheReadResp(StandardMem::ReadResp* resp)
{
    auto it = requests_.find(resp->getID());
    if (it == requests_.end()) {
...
void QuetzTestCPU::onMmioWriteReq(StandardMem::Write* write)
{
    if (!write->posted) {
...
void QuetzTestCPU::onMmioWriteResp(StandardMem::WriteResp* resp)
{
    auto it = requests_.find(resp->getID());
    if (it == requests_.end()) {
...
void QuetzTestCPU::onMmioReadResp(StandardMem::ReadResp* resp)
{
    auto it = requests_.find(resp->getID());
    if (it == requests_.end()) {
...
void QuetzTestCPU::completeCudaCall(const BalarCudaCallReturnPacket_t* ret_pack)
{
    ...
    if (ret_pack->cuda_call_id == CUDA_REG_FAT_BINARY && trace_parser_) {
        ...
        if (sim_ptr && real_ptr) {
            ...
            // Mirrors BalarTestCPU; filenames carry the pointers + size.
...
void QuetzTestCPU::tryFinishSimulation()
{
    if (mode_ == DriverMode::DOORBELL_ONLY) {
        if (!packet_issue_active_ && requests_.empty() && calls_completed_ >= 1) {
            out.verbose(CALL_INFO, 1, 0, "QuetzTestCPU: Test Completed Successfuly\n");
...
bool QuetzTestCPU::clockTic(Cycle_t)
{
    ++clock_ticks_;

    if (mode_ == DriverMode::DOORBELL_ONLY) {
        // TODO: replace clock_ticks_==1 magic-edge with an explicit issued_initial_packet_ flag
        // (or kick from setup()), so the trigger doesn't depend on tick numbering.
        if (requests_.empty() && !packet_issue_active_ && clock_ticks_ == 1) {
            BalarCudaCallPacket_t fatbin{};
            fatbin.cuda_call_id = CUDA_REG_FAT_BINARY;
            fatbin.isSSTmem = false;
            strncpy(fatbin.register_fatbin.file_name, cuda_executable_.c_str(), BALAR_CUDA_MAX_FILE_NAME - 1);
