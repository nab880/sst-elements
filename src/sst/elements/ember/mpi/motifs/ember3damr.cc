...
bool Ember3DAMRGenerator::generate( std::queue<EmberEvent*>& evQ)
{
	if(iteration < maxIterations) {
		// nsx: capture start time before the first iteration's communication.
...
//		}
		...
		if(nextBlockToBeProcessed == localBlocks.size()) {
			...
			// nsx: capture stop time after the final iteration's wait-all.
			if(iteration == maxIterations) {
...
//		iteration++;
		...
		// nsx: parseable per-iteration latency (rank 0), keyed by the block face
		// message size (items_per_cell * face cells * 8 B). Matches the
		// "loop N, bytes B, latency X us" form the harness parses.
		if( 0 == rank() ) {
			double per_iter_us =
