...
bool EmberSweep3DGenerator::generate( std::queue<EmberEvent*>& evQ) {
	...
	// Terminal call: print per-directional-sweep latency (rank 0) once all
	// sweeps complete (start/stop captured via enQ_getTime).
	...
	if( ++m_InnerLoopIndex == 4 ) {
    	if ( ++m_loopIndex == (iterations * 2) ) {
    		// Capture stop time; the next (terminal) call prints and returns true.
