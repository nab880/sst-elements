...
bool EmberFFT3DGenerator::generate( std::queue<EmberEvent*>& evQ )
{
    ...
    if (  m_loopIndex == (signed) m_iterations ) {
        if ( 0 == rank() ) {
            ...
            // nsx: parseable per-iteration (fwd+bwd) latency, keyed by the
            // per-rank FFT volume (np^3/ranks complex doubles = *16 B) as the
            // size proxy. Matches the "loop N, bytes B, latency X us" form the
            // harness parses.
            double per_iter_us =
