...
void QemuMemAccessHandler::handle(unsigned int vcpu_index,
    ...
    if (g_mmio_sync_size != 0 && vaddr >= g_mmio_sync_base &&
        ...
        // Doorbell/status MMIO: flush pending compute so the drain-gate sees the
        // same pre-doorbell instruction stream it would without coalescing.
...
#warning "QEMU plugin API < 4: store-data capture is unavailable; build against a QEMU with plugin API v4+ for balar packet / store payloads."
    ...
    flush_run(vcpu_index);    // keep program order: compute run precedes this memop
