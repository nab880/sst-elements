...
enum QuetzShmemCmd : uint32_t {
    ...
    // Synchronous cache operation: size=0 MOVEC (addr=control register, value=register value), size=1 CPUSHL (addr=An set/way, value=instruction).
...
struct QuetzSharedData {
    ...
    uint32_t          _pad2;
    // QEMU publishes each CPU reset before that CPU can resume execution. SST acquire-loads the epoch before consuming the CPU's next request.
    volatile uint32_t cpu_reset_epoch[QUETZ_MAX_MMIO_VCORES];
    uint32_t local_ram_offset;
    // QEMU maps aligned P1/P2 RAM directly from this backing.
