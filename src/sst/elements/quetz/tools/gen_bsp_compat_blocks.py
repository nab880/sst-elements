...
# Keep the profile target "raptor" independent of the specific board layout.
COMPAT_TARGET = "raptor"


# Generate separate compat, DTIM and GPIO ownership tables to prevent overlapping apertures.
COMPAT_MODEL = "compat"
DTIMER_MODEL = "dtimer"
GPIO_MODEL = "gpio"
KNOWN_MODELS = (COMPAT_MODEL, DTIMER_MODEL, GPIO_MODEL)

# Per dedicated-device-model header parameters: (guard, target-macro, struct name, array name, human-readable description of the model).
DEVICE_HEADERS = {
    DTIMER_MODEL: {
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.print_json:
            # Discovery (bsp_profile.py) wants every bsp_compat block regardless of which device models it, so it can still name DTIM/GPIO accesses.
