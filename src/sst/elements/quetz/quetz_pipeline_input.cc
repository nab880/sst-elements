...
public:
    ...
    bool refill(std::queue<PipelineEvent>& q,
        ...
        bool saw_exit = false;
        // Finish expanding a compute run carried over from a previous tick first,
        // so the per-tick fill rate matches the un-coalesced stream exactly.
        drainRun(q, max_queue_len);

        while (q.size() < max_queue_len && run_remaining_ == 0) {
            QuetzCommand cmd;
            if (!backend_->readCommandNB(core_id_, &cmd))
                break;

            // A compute run stands in for `size` identical NOP events; rebuild
            // them so every downstream stat increments as if uncoalesced.
            if (cmd.cmd == QUETZ_CMD_COMPUTE_RUN) {
                uint32_t cls = cmd.insn_class;
                run_nop_       = cmd;
                run_nop_.cmd   = QUETZ_CMD_NOP;
                run_nop_.size  = 0;
                run_stall_     = (cls < (uint32_t)QUETZ_INSN_CLASS_COUNT)
                                   ? compute_latency_[cls] : 0;
                run_remaining_ = cmd.size ? cmd.size : 1;
                drainRun(q, max_queue_len);
                continue;
            }
...
private:
    void drainRun(std::queue<PipelineEvent>& q, uint32_t max_queue_len) {
        while (run_remaining_ > 0 && q.size() < max_queue_len) {
            q.push({ run_nop_, run_stall_ });
            run_remaining_--;
        }
    }
    ...
    QuetzCommand      run_nop_{};        // NOP template for the in-flight run
    uint32_t          run_remaining_ = 0;
    uint32_t          run_stall_     = 0;
};
