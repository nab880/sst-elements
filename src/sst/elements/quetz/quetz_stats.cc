...
void QuetzCoreStats::registerAll(QuetzCore* comp, const char* sub_id) {
    ...
    async_submits         = comp->registerStatistic<uint64_t>("async_submits",       sub_id);
    async_completions     = comp->registerStatistic<uint64_t>("async_completions",   sub_id);
    async_overlap_cycles  = comp->registerStatistic<uint64_t>("async_overlap_cycles", sub_id);
