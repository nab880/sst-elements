...
class QuetzCPU : public SST::Component, public AcceleratorHost {
public:
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        { "balar_packet_flush_bytes",
          ...
          "4096" },
        { "async_offload",
          "Set to 1 to enable P4 asynchronous offload: writes to the async "
          "submit aperture post the balar doorbell and return a ticket "
          "immediately, and the guest polls a completion counter rather than "
          "blocking for the whole GPU op. Default 0 (synchronous offload).",
          "0" },
        { "async_doorbell_addr",
          "Base MMIO address of the async submit/completion aperture (distinct "
          "from balar_doorbell_addr). Register map: SUBMIT(+0x00,W), "
          "TICKET(+0x08,R), COMPLETED(+0x10,R), STATUS(+0x18,R), "
          "RESULT(+0x20,R). Required when async_offload=1.",
          "0" },
        { "async_doorbell_size",
          "Size in bytes of the async submit/completion aperture.",
          "0x40" },
    ...
    SST_ELI_DOCUMENT_STATISTICS(
        ...
        { "mmio_doorbell_flush_cycles",
          ...
          "cycles", 1 },
        { "async_submits",
          "P4 posted (asynchronous) offloads submitted via the async aperture.",
          "requests", 1 },
        { "async_completions",
          "P4 posted offloads that completed (balar released its deferred "
          "doorbell response).",
          "requests", 1 },
        { "async_overlap_cycles",
          "Cycles a vCPU advanced while one of its posted offloads was still in "
          "flight — the CPU/GPU overlap the async path enables.",
          "cycles", 1 },
        { "read_request_sizes",
        ...
        { "filtered_writes",
          ...
          "requests", 1 },
        { "gpu_doorbell_writes",
          "Writes to the GPU doorbell register (GpuTraceRegionHandler).",
          "requests", 1 },
        { "gpu_status_polls",
          "Reads of the GPU status register (GpuTraceRegionHandler).",
          "requests", 1 },
        { "gpu_other_reads",
          "Other reads in a GpuTraceRegionHandler MMIO range.",
          "requests", 1 },
        { "gpu_other_writes",
          "Other writes in a GpuTraceRegionHandler MMIO range.",
          "requests", 1 },
        { "stall_cycles",
    ...
    SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS(
        ...
        { "accelerator",
          "Host-side accelerator port(s): doorbell aperture, coherence-flush "
          "policy, and posted/async completion engine. If left empty but "
          "balar_doorbell_addr is set, a default quetz.BalarAcceleratorPort is "
          "created from the balar_*/async_* params (back-compat).",
    ...
    // AcceleratorHost — services the accelerator port(s) call back into.
    void     postResponse(uint32_t vcpu, uint64_t value) override;
    void     sendMem (uint32_t vcpu, SST::Interfaces::StandardMem::Request* r) override;
    void     sendMmio(uint32_t vcpu, SST::Interfaces::StandardMem::Request* r) override;
    bool     isDrained(uint32_t vcpu) const override;
    bool     hasMem(uint32_t vcpu)  const override;
    bool     hasMmio(uint32_t vcpu) const override;
    uint64_t cacheLineSize() const override;
    uint64_t cycles() const override;
    void     recordSyncRequest(uint32_t vcpu, bool is_read) override;
    void     recordDoorbellFlush(uint32_t vcpu) override;
    void     recordDoorbellFlushCycles(uint32_t vcpu, uint64_t c) override;
    void     recordAsyncSubmit(uint32_t vcpu) override;
    void     recordAsyncCompletion(uint32_t vcpu) override;
...
private:
    ...
    // True if vCPU `vcpu` has a posted offload that has not yet completed.
