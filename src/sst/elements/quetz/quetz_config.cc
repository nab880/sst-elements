...
QuetzConfig QuetzConfig::fromParams(Params& params, SST::Output* out) {
    ...
    cfg.async_offload =
        params.find<bool>("async_offload", false);
    cfg.async_doorbell_addr =
        params.find<uint64_t>("async_doorbell_addr", 0);
    cfg.async_doorbell_size =
        params.find<uint64_t>("async_doorbell_size", 0x40);
    cfg.async_completion_depth =
        params.find<uint32_t>("async_completion_depth", 1);

    if (cfg.async_offload && cfg.async_doorbell_addr == 0)
