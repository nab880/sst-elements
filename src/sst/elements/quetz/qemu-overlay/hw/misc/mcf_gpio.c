/* Raptor GPIO: masked value writes, plain direction writes; interrupt arming is unsupported. */
...
/* Host levels affect input pins only; undriven pins retain latch readback. */
                       ...
                       unsigned size)
{
    ...
    switch (offset) {
    case GPIO_VALUE_OFFSET: {
        /* The high byte selects pins; the low byte supplies their values. */
    ...
    case GPIO_DIRECTION_OFFSET:
        /* The BSP already performed the direction read-modify-write. */
    ...
    case GPIO_INT_ENABLE_OFFSET:
        ...
        /* Preserve readback even when unsupported IRQ arming is reported. */
...
static const MemoryRegionOps gpio_ops = {
    ...
    /* Allow wrong-width accesses to reach the diagnostic handler. */
