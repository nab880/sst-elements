...
struct QuetzConfig {
    ...
    // P4 asynchronous offload. When async_offload is set, writes to the async
    // submit aperture post the balar doorbell and return immediately (with a
    // ticket); the guest polls a completion counter instead of blocking for the
    // whole GPU op. Default off — the synchronous path is unchanged.
    bool     async_offload          = false;
    uint64_t async_doorbell_addr    = 0;
