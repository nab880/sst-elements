...
bool QuetzCore::isDrained() const {
    return pipeline_->isDrained();
}
...
void QuetzCore::recordAsyncSubmit() {
    stats_.async_submits->addData(1);
}

void QuetzCore::recordAsyncCompletion() {
    stats_.async_completions->addData(1);
}

void QuetzCore::recordAsyncOverlapCycle() {
    stats_.async_overlap_cycles->addData(1);
}
