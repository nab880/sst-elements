...
public:
    ...
    // All backing regions share capacity, tags and controls; actions identify their backing addresses.
    ...
    // Reset preserves cache tags/data (MCF5485RM 7.10.1); invalidate before enabling stale data.
    void reset() {
    ...
    // Supervisor data controls only; instruction controls are inert and unsupported data modes fail closed.
    ...
    // CPUSHL selects An[12:4] set and An[1:0] way; DDPI retains valid lines and DEC does not gate it.
            ...
            while (offset_ < count_) {
                ...
                if (slot == NoSlot) {
                    ...
                    if (old.valid && old.dirty) {
                        // Complete dirty victim writeback before refill; no hardware overlap or latency claim.
    ...
    void complete(const std::vector<uint8_t>& bytes = {}) {
        ...
        if (pending_ == Pending::Fill) {
            ...
            // WD4 clears M on write-through hits; clean copyback data before switching modes or dirty bytes can be lost.
