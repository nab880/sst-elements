...
`tests/manual/validate_against_libmem.py` is a manual developer tool (not part of the auto-discovered testsuite) that compares Quetz reconciled load/store counts against QEMU’s reference `libmem.so` plugin (same guest: vanadis RISC-V hello). Soft tolerance is 5% until syscall-emulation deltas are baselined.

```bash
export SST_HOME=/opt/sst
export QEMU_PLUGIN_DIR=/opt/qemu/lib/qemu/plugins
python3 src/sst/elements/quetz/tests/manual/validate_against_libmem.py
```
