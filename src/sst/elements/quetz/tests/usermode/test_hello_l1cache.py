#!/usr/bin/env python3
"""
SST test: QuetzComponent with a 32 KiB L1 data cache.

Memory hierarchy:
  QuetzComponent → L1Cache (MSI, 32 KiB, 4-way) → MemController (simpleMem)

Verifies that the cache subcomponent wiring works end-to-end.

Uses the pre-compiled hello-world binary from the vanadis test suite.

What the test verifies:
  - Component initialises and QEMU attaches.
  - Cache is populated (CacheHits + CacheMisses > 0).
  - read_requests > 0, write_requests > 0.

To run:
  sst test_hello_l1cache.py
"""
