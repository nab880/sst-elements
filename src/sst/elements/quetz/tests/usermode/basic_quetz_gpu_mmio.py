"""
basic_quetz_gpu_mmio.py — P6 user-mode synchronous MMIO round-trip SDL.

Same QuetzGpuDevice + mmio_link wiring as sysmode/basic_quetz_gpu.py, but the CPU
runs in user mode (system_mode=0). There is no QEMU device map: the launcher
passes -sst-mmio-range, qemu-<arch> reserves the aperture PROT_NONE, and the
linux-user SIGSEGV handler routes faulting loads/stores to the same sync mailbox
that drives pollMmioSyncMailbox -> handleMmioSyncCommand -> mmio_link_0.

Env (set by the test harness):
  QUETZ_EXE / QUETZ_QEMU / QUETZ_PLUGIN
  QUETZ_MMIO_START / QUETZ_MMIO_END   aperture window (must match GPU base_addr)
  QUETZ_MMIO_PAYLOAD=1                 enables the launcher's -sst-mmio-range
"""
