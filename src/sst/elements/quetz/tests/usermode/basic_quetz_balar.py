"""
basic_quetz_balar.py — P6 user-mode SDL for Quetz -> Balar/GPGPU-Sim wiring.

Same balar block + coherent fabric as sysmode/basic_quetz_balar.py, but the CPU
runs a normal Linux RV64 process (system_mode=0). There is no QEMU device map:
the launcher emits -sst-mmio-range, qemu-riscv64 reserves the balar MMIO window
PROT_NONE, and the linux-user SIGSEGV handler routes the doorbell load/store to
the sync mailbox -> BalarAcceleratorPort (flush + forward to balar). The guest VA
is the SST memory address, so balar's DMA of the packet/H2D/D2H buffers reads the
same bytes the trace stream wrote (the doorbell-flush keeps them coherent).

Run the guest with `-R 0x100000000` (QUETZ_QEMU_ARGS) so the stack lands in the
high RAM region [0x80000000, 0xFFFFFFFF] and the static image in the low region.
"""
...
qemu_args = os.environ.get("QUETZ_QEMU_ARGS", "-R 0x100000000")
...
if qemu_args:
    cpu_params["qemu_args"] = qemu_args
...
# User mode has no UART/testdev/boot-ROM; the only special window is balar's MMIO
# (trapped by QEMU, not the trace path). Everything else is plain RAM.
