...
Manual developer tool — NOT part of the automated testsuite (the SST harness
only auto-discovers testsuite_*.py). Run it by hand to cross-check Quetz's
reconciled load/store counts against QEMU's reference libmem.so plugin.
...
# This script lives in tests/manual/; the SDL decks and gold tree are one level
# up under tests/.
QUETZ_TESTS = os.path.dirname(SCRIPT_DIR)
