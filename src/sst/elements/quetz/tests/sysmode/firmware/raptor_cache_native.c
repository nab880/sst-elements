/* Probe native data caching and a live cached stack; instruction-cache coherence is outside this diagnostic. */
#include <stdint.h>
...
__asm__(".section .text.start,\"ax\"\n.global _start\n_start:\n"
        ...
        "move.l %a2,%sp\nmove.l (%sp)+,%a2\nrts\n"
        /* Clean the live frame and return address without stack writes before DCINVA. */
        ...
        /* Five tags compete for set 257 with no intervening data accesses to that set. */
...
void diagnostic(void) {
    ...
    /* AMM changes coarse 0x80 matching to 0x801xxxxx, excluding native P1 RAM. */
    acr(0x8010c020u);
