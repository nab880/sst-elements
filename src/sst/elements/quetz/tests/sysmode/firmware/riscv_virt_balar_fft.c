/*
 * riscv_virt_balar_fft.c — bare-metal Balar/GPGPU-Sim doorbell driver for a
 * GPU-driven staged radix-2 FFT, issued from a QEMU sysmode guest under Quetz.
 *
 * This is the "GPU-driven FFT through Quetz" demo: the guest marshals
 * BalarCudaCallPacket_t packets into guest DRAM and rings balar.balarMMIO for
 * a bit-reversal kernel followed by log2(N) butterfly-stage kernels (the same
 * fft.cu kernels the trace tests replay).  Each launch re-exercises the host
 * scratch-write / flush / doorbell / DMA path that Quetz models.
 *
 * Correctness gate: a unit-impulse input has X[k] = 1+0j for every bin, which
 * is bit-exact in float32 (twiddles always multiply zero), so the guest checks
 * every output word against 0x3F800000 (1.0f) / 0x00000000 (0.0f) with no
 * tolerance.  Twiddles are precomputed (no libm in firmware) in the generated
 * header below — regenerate with:
 *     fft_reference.py -n 256 --header fft_firmware_data_256.h
 *
 * Build (from firmware/):  see build.sh (RV64 musl-gcc, -T link_rv64.ld).
 */
...
#define FFT_COMPLEX   FFT_N                 /* N complex points              */
#define VEC_WORDS     (FFT_N * 2u)          /* re,im per point (uint32 each) */
#define VEC_BYTES     (VEC_WORDS * 4u)      /* N complex float32             */
#define TW_WORDS      (FFT_N)               /* N/2 complex = N words         */
...
/* Kernel host-function ids (arbitrary, must match register vs launch). */
...
static uint64_t dev_a;     /* input            */
static uint64_t dev_b;     /* working / output */
static uint64_t dev_tw;    /* twiddles         */
...
{
    /* Unit impulse: x[0] = 1.0f + 0j, rest zero. */
...
static void packet_memcpy_d2h(void *dst, uint64_t src, uint64_t nbytes)
{
    ...
    /* Drain the D2H payload from the compact MMIO return path, 8 bytes/read. */
...
/* Launch fft_bitrev(out=dev_b, in=dev_a, N, logn). */
...
/* Launch fft_stage(data=dev_b, tw=dev_tw, N, s, logn) in place. */
