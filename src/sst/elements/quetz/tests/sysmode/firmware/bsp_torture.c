...
void kernel_main(void)
{
    ...
    /* Probe BSP-init ranges from boards/raptor/board.json, not the stock MCF5208 map. */
    ...
    /* Test sticky SCM2 profile readback; GPIO has separate masked-write semantics. */
    ...
    /* A 32-bit access must not match the profile's 16-bit SCM2 register. */
    f = cf_probe_r32(0xFC040020, &v);
    uart_puts("compat width-check: r32 read=");
    uart_put_u32_hex(v);
    uart_putc('\n');

    /* Bound the SCM2 status poll so RAZ cannot hang the firmware. */
