"""Native P1 CPU copyback data and eDMA raw backing visibility diagnostic.

Native P1 RAM has shared raw backing. The CPU data cache and eDMA access that
backing independently; trace memory provides timing only. The stack is SRAM.
"""
import os
import sst
...
for path in (primary,):
    if not os.path.isfile(path) or any(char in path for char in ', \t\n'):
        raise ValueError('eDMA cache diagnostic requires an existing ELF path without QEMU delimiters')
...
cpu.addParams({'verbose': 1, 'clock': '1GHz', 'vcpu_count': 1,
               ...
               'qemu_args': '-machine raptor-core2 '
...
router.addParams({'xbar_bw': '128GB/s', 'id': '0', 'input_buf_size': '1KB',
                  'num_ports': '2', 'flit_size': '72B', 'output_buf_size': '1KB',
...
sst.Link('directory_network').connect((network, 'port', '1ns'), (router, 'port1', '1ns'))
