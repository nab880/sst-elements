...
# P4 async-offload: enable a Quetz-emulated submit/completion aperture so the
# guest can post a blocking balar call (e.g. cudaThreadSynchronize) and poll for
# completion instead of stalling the vCPU. The aperture sits in unused space
# inside balar's MMIO region (the guest only ever touches offset 0), so the QEMU
# bridge already covers it and balar never sees it — Quetz intercepts first.
...
balar_mmio_nic.addParams({
    ...
    "network_bw": GPU_LINK_BW,  # doorbell crosses the host<->GPU link
})
...
dma_mem_nic.addParams({
    "group": CORE_GROUP,
    "destinations": CORE_DST,
    "network_bw": GPU_LINK_BW,  # GPU DMA host-memory access (H2D/D2H) crosses the link
})
