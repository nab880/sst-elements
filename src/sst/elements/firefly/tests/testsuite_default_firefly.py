...
class testcase_firefly(SSTTestCase):
    ...
    def test_firefly_allreduce_recoverable_error(self):
        ...
        self.assertEqual(1, text.count(
            "Firefly Allreduce accepted RecoverableError: retval=1 state=cleared PASS"))
    ...
    def test_firefly_review_regressions(self):
        ...
        finish = function_body(allreduce, "void AllreduceOffloadFuncSM::finishOffload")
        complete = function_body(allreduce, "void AllreduceOffloadFuncSM::complete")
        self.assertIn("logical_endpoint_id", publish)
        self.assertIn("collective->participant_logical_id", publish)
        self.assertIn("physical_endpoint_id = m_myNodeId", publish)
        self.assertIn("injection_dest_nid = collective->root_logical_nid", publish)
        self.assertIn("request->src != collective->root_logical_nid", result)
        ...
        self.assertIn("retval.setExit(terminal_error ? 1 : 0)", finish)
    ...
    def test_firefly_collective_mapped_repeat(self):
        ...
        source = source.replace(
            ...
            "class RemappedReorderLinkControl(ReorderLinkControl):\n"
            "    def build(self, comp, slot, slot_num, job_id, job_size, nid, "
            "use_nid_map=False, link=None):\n"
            "        return super().build(comp, slot, slot_num, job_id, job_size, "
            "nid, True, link)\n\n"
            "networkif = RemappedReorderLinkControl()")
        source = source.replace(
        ...
        self.assertEqual(4, text.count("Ember Allreduce verify rank "))
