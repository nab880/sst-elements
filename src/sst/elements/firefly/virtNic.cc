...
{
    auto* state = featureState();
    if ( collectiveParticipant() == nullptr || state == nullptr || !state->collective_endpoint ) {
...
{
    auto* state = featureState();
    return collectiveParticipant() == nullptr || state == nullptr ? nullptr :
        &*state->collective_endpoint;
}

const SST::Collective::CollectiveParticipant* VirtNic::collectiveParticipant() const
{
    auto* state = featureState();
    return state == nullptr || !state->collective_endpoint ? nullptr :
        state->collective_endpoint->participant();
}
...
SST::Collective::CollectiveSubmitResult VirtNic::submitCollective(
    const SST::Collective::CollectiveSubmission& submission, Hermes::Vaddr source_address, Hermes::Vaddr result_address)
{
    auto* state = featureState();
    if ( state == nullptr || !state->collective_endpoint ) return SST::Collective::CollectiveSubmitResult::Invalid;
    return state->collective_endpoint->submitWithMemory(submission, source_address, result_address);
}
