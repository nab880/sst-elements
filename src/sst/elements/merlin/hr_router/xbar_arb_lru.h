...
#endif
    {
        arbitrateInputs(inputs, outputs, input_busy, output_busy, progress_vc);
        return true;
    }
        ...
        for ( int i = 0; i < total_entries; ++i ) {
            ...
            if ( input_busy[port] <= 0 && event != nullptr && !processor_owned ) {
                ...
                if ( output < 0 || output >= num_output_ports || output_vc < 0 || output_vc >= num_vcs ) {
                    // Never return early: a partial pass would leave every
                    // later entry ungranted and silently stall the crossbar.
                    getSimulationOutput().fatal(CALL_INFO, 1,
                        "xbar_arb_lru: head on input %d VC %d targets invalid output port %d VC %d "
                        "(router has %d output ports and %d VCs)\n",
                        port, vc, output, output_vc, num_output_ports, num_vcs);
                }
