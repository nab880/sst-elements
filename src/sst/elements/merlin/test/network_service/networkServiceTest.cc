...
public:
    ...
    bool spaceToSend(int vc, int flits) override { return credits_ && vc == 0 && flits > 0; }
...
void
testLongPacketLRUFairness()
{
    // Thirteen inputs, two of them holding thirteen-flit packets for the
    // same output.  Output occupancy equal to the input count must not
    // phase-lock the LRU so that one requester never wins.
    constexpr int count = 13;
    std::array<FakeXbarPort, count> ports;
    ...
    for ( int cycle = 0; cycle < 80; ++cycle ) {
        require(arbiter_api.arbitrateNetworkService(inputs.data(), outputs.data(), input_busy, output_busy, progress),
            "long-packet LRU arbitration failed");
        for ( int input = 0; input < count; ++input ) {
            require(progress[input] < 0 || input == 0 || input == count - 1, "empty LRU input received a grant");
        }
